...
export class LaunchtubeStrategy implements SponsorshipStrategy {
  ...
  constructor(private configService: ConfigService) {
    this.launchtubeApiKey =
      this.configService.get<string>("LAUNCHTUBE_API_KEY") || "";
  }

  async sponsorAndSubmit(signedXdr: string): Promise<SponsorshipResult> {
    if (!this.launchtubeApiKey) {
      throw new HttpException(
        "Launchtube API key not configured",
    ...
    try {
      ...
      throw new HttpException(
        `Failed to communicate with Launchtube: ${error.message}`,
        HttpStatus.BAD_GATEWAY,
      );
    }
  }
