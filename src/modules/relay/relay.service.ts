...
import { rpc, TransactionBuilder, Networks, Transaction } from '@stellar/stellar-sdk';
import { LaunchtubeStrategy } from './strategies/launchtube.strategy';
import { SubmitTransactionDto, SubmitTransactionResponseDto } from '@common/schemas/relay.schema';
...
export class RelayService {
  ...
  async submitTransaction(dto: SubmitTransactionDto): Promise<SubmitTransactionResponseDto> {
    try {
      // 1. Validate XDR shape
      // We parse the transaction to ensure it's a valid XDR and extract the source account (wallet address)
      const tx = TransactionBuilder.fromXDR(dto.signedXdr, Networks.TESTNET) as Transaction;
      ...
      const result = await this.launchtubeStrategy.sponsorAndSubmit(dto.signedXdr);
      ...
      throw new BadRequestException(`Failed to process transaction: ${error.message}`);
    }
  }
  ...
  async getTransactionStatus(txHash: string) {
    try {
      ...
      throw new BadRequestException(`Failed to fetch transaction status: ${error.message}`);
    }
  }
}
