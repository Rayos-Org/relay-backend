...
export class IndexerService implements OnApplicationBootstrap {
  ...
  private async fetchAndProcessEvents() {
    ...
    let startLedger = storedCursor ? parseInt(storedCursor, 10) : latestLedger;
  ...
  private async processEvent(event: rpc.Api.EventResponse) {
    ...
    await this.db.insert(schema.credentialLookup)
      .values({
    ...
    this.logger.log(`Indexed credential ${credentialId} for wallet ${walletAddress}`);
  }

  async getWalletByCredential(credentialId: string) {
    const [lookup] = await this.db.select()
