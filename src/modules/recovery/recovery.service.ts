...
import { ProposeRecoveryDto, ApproveRecoveryDto } from '@common/schemas/recovery.schema';
...
export class RecoveryService {
  ...
  async proposeRecovery(dto: ProposeRecoveryDto) {
    ...
    timelockExpiresAt.setHours(timelockExpiresAt.getHours() + this.TIMELOCK_HOURS);
    ...
    const guardians = ['guardian_address_1', 'guardian_address_2', 'guardian_address_3']; // Mocked
    ...
    // Schedule expiration
    await this.recoveryQueue.add('expire-proposal', { proposalId }, { delay: this.TIMELOCK_HOURS * 3600 * 1000 + 3600000 }); // Expire 1 hour after timelock

    return { proposalId, timelockExpiresAt: timelockExpiresAt.toISOString() };
  }

  async approveRecovery(dto: ApproveRecoveryDto) {
    const [proposal] = await this.db.select().from(schema.recoveryProposals).where(eq(schema.recoveryProposals.proposal_id, dto.proposalId)).limit(1);
    ...
    if (approvals.length >= this.GUARDIAN_THRESHOLD) {
      ...
      await this.recoveryQueue.add('execute-recovery', { proposalId: dto.proposalId }, { delay });
    }
